...
* Expected Risk(예상 손해)
    ...
    * $ R(\alpha_i|x) = \text{입력을} c_i \text{로 분류했을 때의 위험부담} \equiv \sum_k \lambda_{ik}P(c_k|x) $
        
        = (입력이 i를 제외한 class일 확률) $\times$ (그 상황에서의 손해) 의 총합

- $ \text{즉, Bayesian Decision Rule은} i = \underset{i}{\arg\max}R(\alpha_i|x) \text{를 골라} \alpha_i $
